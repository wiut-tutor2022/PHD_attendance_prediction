...
###################################
data_input = df.drop(['Unnamed: 0', 'date', 'month', 'year','attendance', 'attendance_by_class','normalized_attendance', 'class_type_new', 'day', 'date-year', "date-month"], axis=1)  #
...
final_result = neural_network.think(np.array(
    [week,
     # date,
      room_name, time_of_day, class_type, faculty, school, joint, status, degree, enrollment, class_duration, date_day
     ])),
